function r = panda_model()
    ...
    robot = SerialLink([L1 L2 L3 L4 L5 L6 L7], 'name', 'PANDA', 'manufacturer', 'Franka-Emika', 'tool', transl([0 0 0*mm]));

    qv = [0 0 0 0 0 0 0]*deg;
    qr = [0 -90 -90 90 0 -90 90]*deg;
    qh = [0 0 0 -90 0 0 0]*deg; %Home Configuartion
    M = [[1, 0, 0, 0.088];[0, -1, 0, 0];[0, 0, -1, 0.926];[0, 0, 0, 1]];
    B = [[0;0;-1;0;-0.088;0],[0;-1;0;0.316+0.384-0.107;0;0.088],[0;0;-1;0;-0.088;0],[0;1;0;-(0.384-0.107);0;-(0.088-0.0825)],[0;0;-1;0;-0.088;0],[0;1;0;0.107;0;-0.088],[0;0;1;0;0;0]];
    ...
    elseif nargin == 0
        ...
        assignin('caller', 'qh', qh); % home configuration
        assignin('caller', 'M', M);
        assignin('caller', 'B', B);
        assignin('caller', 'S', S);
    end
